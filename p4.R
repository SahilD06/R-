...
# Quick look at the data structure (Results not shown here, but available)
head(csv2)

### Example 1: Single Condition
# Filter for all students who are **older than 20** (Age > 20).
...
head(female_older_20_filter)
  ...
  # Filter for students who have either **Anxiety** OR a **Panic Attack**.
...
head(anxiety_or_panic_filter)
  ...
  ### Example 4: Checking for Values in a Set ($\texttt{\%in\%}$)
  # Filter for students in their **year 1** or **year 2** of study.
  
year_1_or_2_filter <- csv2 |>
filter(`Year` %in% c("1", "2"))

cat("Number of students in Year 1 or Year 2:", nrow(year_1_or_2_filter), "\n")
table(year_1_or_2_filter$`Your current year of Study`)
